...
from qgis.PyQt.QtWidgets import QDialog, QDialogButtonBox, QCheckBox, QMessageBox, QGraphicsScene, QGraphicsEllipseItem
...
class LeerrohrVerlegenTool(QDialog):
    ...
    def populate_farbschema(self):
        ...
        firma = self.ui.comboBox_Firma.currentData()
        ...
        try:
            ...
            cur.execute("""
                SELECT DISTINCT c."CODIERUNG", ls."ID_CODIERUNG"
                FROM lwl."LUT_Leerrohr_SubTyp" ls
                JOIN lwl."LUT_Codierung" c ON ls."ID_CODIERUNG" = c."id"  -- Änderung hier: c."ID_CODIERUNG" → c."id"
                WHERE ls."FIRMA" = %s AND ls."ID_TYP" = %s
                ORDER BY c."CODIERUNG";
            """, (firma, typ_id))

            rows = cur.fetchall()
            print(f"DEBUG: Gefundene Codierungen und IDs: {rows}")

            if rows:
                self.ui.comboBox_Farbschema.setEnabled(True)
                for codierung, codierung_id in rows:
                    self.ui.comboBox_Farbschema.addItem(codierung, codierung_id)
                self.ui.comboBox_Farbschema.setCurrentIndex(0)
            else:
    ...
    def populate_leerrohr_subtypen(self):
        ...
        firma = self.ui.comboBox_Firma.currentData()
        typ_id = self.ui.comboBox_leerrohr_typ.currentData()
        codierung_id = self.ui.comboBox_Farbschema.currentData()

        print(f"DEBUG: Eingaben für Subtypen – Firma: {firma}, Typ_ID: {typ_id}, Codierung_ID: {codierung_id}")
        ...
        try:
            ...
            print(f"DEBUG: Lade Subtypen – FIRMA: {firma}, ID_TYP: {typ_id}, ID_CODIERUNG: {codierung_id}")
            cur.execute("""
                SELECT ls."id", ls."SUBTYP_char", ls."BEZEICHNUNG"
                FROM lwl."LUT_Leerrohr_SubTyp" ls
                JOIN lwl."LUT_Codierung" c ON ls."ID_CODIERUNG" = c."id"  -- Änderung hier: c."ID_CODIERUNG" → c."id"
                WHERE ls."FIRMA" = %s AND ls."ID_TYP" = %s AND ls."ID_CODIERUNG" = %s;
            """, (firma, typ_id, codierung_id))

            rows = cur.fetchall()
            print(f"DEBUG: Gefundene Subtypen: {rows}")
    ...
    def parse_rohr_definition(self, subtyp_id):
        """Parses ROHR_DEFINITION (jsonb) aus LUT_Leerrohr_SubTyp und gibt eine Liste von Rohr-Informationen zurück."""
        print(f"DEBUG: Parsing ROHR_DEFINITION (jsonb) für Subtyp-ID: {subtyp_id}")
        db_details = self.get_database_connection()
        conn = None
        cur = None
        rohre = []
        ...
        try:
            ...
            # Hole ROHR_DEFINITION und ID_CODIERUNG aus LUT_Leerrohr_SubTyp
            cur.execute("""
                SELECT ls."ROHR_DEFINITION"::text, ls."ID_CODIERUNG"
                FROM lwl."LUT_Leerrohr_SubTyp" ls
                WHERE ls."id" = %s
            """, (subtyp_id,))
            result = cur.fetchone()
            if not result:
                raise ValueError(f"Subtyp-ID {subtyp_id} nicht gefunden in LUT_Leerrohr_SubTyp.")

            rohr_definition, id_codierung = result
            if not rohr_definition:
                raise ValueError(f"ROHR_DEFINITION ist leer für Subtyp-ID {subtyp_id}.")

            # Parse ROHR_DEFINITION (jsonb)
            import json
            rohr_array = json.loads(rohr_definition)

            # Berechne die Gesamtzahl der Rohre und erstelle Rohrliste
            total_rohre = 0
            rohr_liste = []
            for rohr in rohr_array:
                anzahl = int(rohr.get("anzahl", 1))
                durchmesser = float(rohr.get("durchmesser", 0))
                for j in range(anzahl):
                    total_rohre += 1
                    rohr_liste.append((total_rohre, durchmesser))
            print(f"DEBUG: Gesamtzahl der Rohre: {total_rohre}, Rohrliste: {rohr_liste}")

            # Hole Farben aus LUT_Farbe_Rohr
            cur.execute("""
                SELECT fr."ROHRNUMMER", fr."FARBE", fr."FARBCODE", fr."CODIERUNG"
                FROM lwl."LUT_Farbe_Rohr" fr
                WHERE fr."ID_CODIERUNG" = %s
                ORDER BY fr."ROHRNUMMER"
            """, (id_codierung,))
            farben = cur.fetchall()
            if not farben:
                raise ValueError(f"Keine Farben gefunden für ID_CODIERUNG {id_codierung} in LUT_Farbe_Rohr.")

            # Prüfe, ob genügend Farben vorhanden sind
            max_farbnummer = max(f[0] for f in farben)
            if total_rohre > max_farbnummer:
                codierung = farben[0][3]  # CODIERUNG aus dem ersten Eintrag
                raise ValueError(
                    f"Unzureichende Farbanzahl in LUT_Farbe_Rohr für CODIERUNG: {codierung}. "
                    f"Benötigt: {total_rohre}, Verfügbar: {max_farbnummer}. Bitte Daten korrigieren oder anlegen."
                )

            # Weise jedem Rohr die entsprechende Farbe zu
            farben_dict = {f[0]: (f[1], f[2]) for f in farben}  # ROHRNUMMER -> (FARBE, FARBCODE)
            for rohr_nummer, durchmesser in rohr_liste:
                farbe, farbcode = farben_dict.get(rohr_nummer, ("grau", "#808080"))  # Sollte nicht vorkommen wegen Prüfung
                rohr_id = rohr_nummer  # Nur ROHRNUMMER als ID
                rohre.append((rohr_id, rohr_nummer, durchmesser, farbe, farbcode))

            print(f"DEBUG: Gefundene Rohre mit Farben: {rohre}")
            return rohre

        except json.JSONDecodeError as e:
            self.ui.label_Status.setText(f"Fehler beim Parsen von ROHR_DEFINITION: {str(e)}")
            self.ui.label_Status.setStyleSheet("background-color: lightcoral;")
            print(f"DEBUG: Fehler beim Parsen des JSON: {e}")
            return []
        except ValueError as e:
            self.ui.label_Status.setText(str(e))
            self.ui.label_Status.setStyleSheet("background-color: lightcoral;")
            print(f"DEBUG: Fehler: {e}")
            return []
        except Exception as e:
            self.ui.label_Status.setText(f"Fehler beim Parsen der Rohre: {str(e)}")
            self.ui.label_Status.setStyleSheet("background-color: lightcoral;")
    ...
    def update_rohr_anzeige(self):
        """Aktualisiert die Anzeige der Rohre in graphicsView_Auswahl_Rohre."""
        print("DEBUG: Starte update_rohr_anzeige")
        if self.ui.graphicsView_Auswahl_Rohre.scene():
            self.ui.graphicsView_Auswahl_Rohre.scene().clear()
        else:
            self.ui.graphicsView_Auswahl_Rohre.setScene(QGraphicsScene())

        # Bestimme den Subtyp basierend auf dem Modus
        subtyp_id = None
        if self.ui.radioButton_Hauptstrang.isChecked():
            subtyp_id = self.ui.comboBox_leerrohr_typ_2.currentData()
            if not subtyp_id:
                self.ui.label_Status.setText("Bitte Subtyp auswählen.")
                return
        elif self.ui.radioButton_Abzweigung.isChecked():
            if not self.selected_parent_leerrohr or "SUBTYP_ID" not in self.selected_parent_leerrohr:
                self.ui.label_Status.setText("Kein Parent-Leerrohr ausgewählt.")
                return
            subtyp_id = self.selected_parent_leerrohr.get("SUBTYP_ID")

        # Hole die Rohre
        rohre = self.parse_rohr_definition(subtyp_id)
        if not rohre:
            self.ui.label_Status.setText("Keine Rohre zum Anzeigen gefunden.")
            return

        # Erstelle eine neue QGraphicsScene (wenn nicht vorhanden)
        scene = self.ui.graphicsView_Auswahl_Rohre.scene()
        x_offset = 5  # Abstand vom Rand
        y_offset = 5  # Zentriert in 51px Höhe
        max_height = 51  # Maximale Höhe des graphicsView

        for rohr_id, rohr_nummer, durchmesser, farbe, farbcode in rohre:
            # Skaliere die Größe basierend auf dem Durchmesser
            scale_factor = (50 - 20) / (50 - 7)  # Linear zwischen 20 und 50
            size = min(max_height, 20 + (durchmesser - 7) * scale_factor)
            radius = size / 2

            # Erstelle den Kreis
            circle = QGraphicsEllipseItem(x_offset, y_offset, size, size)
            circle.setBrush(QBrush(QColor(farbcode)))  # Fülle mit Farbe
            circle.setPen(QPen(Qt.black, 1))  # Schwarzer Rand
            circle.setZValue(0)  # Setze Z-Wert des Kreises auf 0 (Hintergrund)

            # Zeige die Rohrnummer an
            text = scene.addText(str(rohr_nummer))
            text_center_x = x_offset + size / 2 - text.boundingRect().width() / 2  # Mitte des Kreises in x-Richtung
            text_center_y = y_offset + size / 2 - text.boundingRect().height() / 2  # Mitte des Kreises in y-Richtung
            text.setPos(text_center_x, text_center_y)
            # Dynamische Schriftfarbe basierend auf der Helligkeit der Kreis-Farbe
            color = QColor(farbcode)
            brightness = (color.red() * 0.299 + color.green() * 0.587 + color.blue() * 0.114)  # Helligkeit berechnen
            text.setDefaultTextColor(Qt.black if brightness > 128 else Qt.white)  # Schwarz für helle, Weiß für dunkle Farben
            font = text.font()
            font.setPointSize(12)  # Schriftgröße bleibt 12
            text.setFont(font)
            text.setZValue(1)  # Setze Z-Wert des Textes auf 1 (Vordergrund)
            print(f"DEBUG: Nummer {rohr_nummer} positioniert bei x={text_center_x}, y={text_center_y}")

            # Füge zur Szene hinzu
            scene.addItem(circle)
            scene.addItem(text)

            # Aktualisiere den Offset für den nächsten Kreis
            x_offset += size + 5  # Abstand zwischen Kreisen

        # Setze die Szene im graphicsView (falls nicht bereits gesetzt)
        self.ui.graphicsView_Auswahl_Rohre.setScene(scene)
        self.ui.graphicsView_Auswahl_Rohre.setHorizontalScrollBarPolicy(Qt.ScrollBarAsNeeded)
        self.ui.graphicsView_Auswahl_Rohre.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
    ...
    def importiere_daten(self):
        ...
        try:
            ...
            if self.ui.radioButton_Abzweigung.isChecked():
                ...
                codierung = self.ui.comboBox_Farbschema.currentText().strip()
                subtyp_id = self.ui.comboBox_leerrohr_typ_2.currentData()
                firma_hersteller = self.ui.comboBox_Firma.currentData()
